            ...
            if (k == null) {
                ...
                synchronized (keyLock) {
                    if (!isOpen())
                        throw new ClosedChannelException();
                    /**
                     * 首先当前类属于 java.nio.channels.spi 这个包下面
                     * 也就是下面的这个实现类根本就是不确定的，取决于平台
                     * 如果自己往里面找的话会发现，selector 的实现类是 WindowsSelectorImpl
                     *
                     * selector 实现的具体的事情就是 epoll_wait(select) 和 epoll_ctl(add)
                     */
