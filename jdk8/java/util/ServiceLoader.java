...
public final class ServiceLoader<S>
    ...
    /**
     * 这个是一个访问的标志，如果是第一次访问就是null，如果不是第一次访问就不是null
     */
     ...
     */
    public void reload() {
        // HashMap 的 clear 方法，把整个集合清空
        providers.clear();
        // 这个LazyIterator应该是非常有用的
    ...
    private class LazyIterator
        ...
        private boolean hasNextService() {
            ...
            if(configs == null) {
                try {
                    ...
                    // service是Interface的class名，这两个拼接起来就是 resources 里面的那个资源文件名
            ...
            /**
             * 文件资源导出来之后，把一个文件里面所有的 URL 导出成一个 Iterator
             * 之后的 hasNext 就用这个 Iterator 来判断
             */
            while((pending == null) || !pending.hasNext()) {
                ...
                /**
                 * 这里是分了两层结构，一层结构是文件会有很多个，第二层结构是文件里面有好多全限定名
                 * 这里的 pending 应该就是用一个文件导出来的，下面的 parse 方法应该是把一个文件变成一个 Iterator 对象
                 */
        ...
        private S nextService() {
            ...
            /**
             * 这里的 nextName 是在上面的 hasNext() 方法里面加载出来的
             * 也就是下面要加载的一个全限定名
             */
            String cn = nextName;
            nextName = null;
            ...
            try {
                /**
                 * 那这个 SPI 也没什么稀奇的了
                 * 也就是接口那一层不知道下面用什么来实现，然后跟下层越好把实现类的全限定名写在某一个位置下的配置文件里面
                 * 然后上层接口那一层去这个配置文件里面读取出全限定名
                 * 通过Class.forName反射把这个类加载进来
                 *
                 * 中间有一次要获取到AppClassLoader，这里破坏了双亲委派
                 */
        ...
        public boolean hasNext() {
            /**
             * 如果是第一次访问的话，acc就会是null，然后进入这个if
             */
            if(acc == null) {
                return hasNextService();
            } else {
                /**
                 * 感觉这里面的还是用 hasNextService() 方法来获取的
                 */
